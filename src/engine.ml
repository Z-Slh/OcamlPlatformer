...
let discx (opx, opy) interp ((npx, width), (npy, height)) (vx, vy) l =
  let diff = (npx,npy) -.. interp in
  ...
  let npx2, vx2 =
    ...
    | None -> (npx, vx)
  in
  (((npx2, npy2) -.. diff : vec), ((vx2, vy2) : vec), (mk_obst h_obst, mk_obst v_obst))
...
let update_physics (pos, s) v fl o_list =
  ...
  (*List.iter
    ...
    int_obst;*)
  discx pos pos2 obj_bound v2 int_obst
